...
sudo docker stop ecdsa-keep-client
sudo docker rm ecdsa-keep-client
sudo docker images -a | grep "ecdsa-keep-client" | awk '{print $3}' | xargs sudo docker rmi
sudo docker pull $KEEP_ECDSA_DOCKER_IMAGE
